...
test('returns winner', () => {
	...
	// Excercise 1:
	// Change the getWinner implementation to a function that keeps track of how often it's called and
	...
	// Excercise 2:
	// Add an assertion for how many times the getWinner function was supposed to be called (2 times) (Hint #2)
	expect(utils.getWinner.mock.calls.length).toBe(2);


	// Excercise 3:
	// Add another assertion that every time it was called it was called with the right arguments: 
	// 'Ken Wheeler', 'Kent C. Dodds' (Hint #3)
	for(let i = 0; i < utils.getWinner.mock.calls.length; i++){
		expect(utils.getWinner.mock.calls[i]).toEqual(['Ken Wheeler', 'Kent C. Dodds']);
	}
